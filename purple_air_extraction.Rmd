...
fire_period = AQ_df %>%
  ...
  mutate(fire_period = datetime>=as.POSIXct(strptime("2021-12-30 10:00:00", "%Y-%m-%d %H:%M:%S")) & datetime<= as.POSIXct(strptime("2022-01-01 11:59:59", "%Y-%m-%d %H:%M:%S")),
         post_fire_period = datetime>=as.POSIXct(strptime("2022-1-2 00:00:00", "%Y-%m-%d %H:%M:%S"))) %>%
  # filter by the fire period to create % completeness within the fire interval
  filter(fire_period == TRUE) %>%
  dplyr::summarize(
    # calculate the % complete for a and b channels (if b channel is less than a channel, do nothing unless using b instead of a achieves appropriate completeness ratio)
    fire_period_pct_complete_a = sum(!is.na(pm25_a))/length(pm25_a)*100,
    fire_period_pct_complete_b = sum(!is.na(pm25_b))/length(pm25_b)*100
  ) %>%
  # create a variable that determines whether there is completeness during the fire period or not
  mutate(fire_period_completeness = ifelse(fire_period_pct_complete_a >= 95 | fire_period_pct_complete_b >= 95,"complete","incomplete")) %>%
  filter(post_fire_period == TRUE) %>%
  dplyr::summarize(
    # calculate the % complete for a and b channels (if b channel is less than a channel, do nothing unless using b instead of a achieves appropriate completeness ratio)
    post_fire_period_pct_complete_a = sum(!is.na(pm25_a))/length(pm25_a)*100,
    post_fire_period_pct_complete_b = sum(!is.na(pm25_b))/length(pm25_b)*100
  ) %>%
  # create a variable that determines whether there is completeness during the fire period or not
  mutate(post_fire_period_completeness = ifelse(post_fire_period_pct_complete_a >= 95 | post_fire_period_pct_complete_b >= 95,"complete","incomplete")) 
...
plot_data = AQ_df %>%
  ...
  dplyr::summarize(first_date = datetime[1],
                   Location = Location[1],
                   Lat = Lat[1],
                   Lon = Lon[1])
...
plot_data = merge(plot_data, fire_period, by = "ID", all=TRUE)

# add status column to plot data based on first date of sensor measurements and fire period completeness; used 10 am because it was right before the fire started
(plot_data = plot_data %>% 
  mutate(status = ifelse(fire_period_pct_complete_a > 80 & first_date<=as.POSIXct("2021-12-30 10:00:00", "%Y-%m-%d %H:%M:%S"), "Complete data throughout the fire period",
                                     ifelse(fire_period_pct_complete_a < 80 & first_date<=as.POSIXct("2021-12-30 10:00:00", "%Y-%m-%d %H:%M:%S"), "Sensor went offline during fire",
                                            ifelse(fire_period_pct_complete_a < 80 & (first_date>=as.POSIXct("2021-12-30 10:00:00", "%Y-%m-%d %H:%M:%S") & first_date <= as.POSIXct("2021-12-31 11:59:59", "%Y-%m-%d %H:%M:%S")), "Sensor added during the fire",
                                            ifelse(first_date>=as.POSIXct("2022-01-01 00:00:00", "%Y-%m-%d %H:%M:%S"), "Sensor added after the fire", NA))))))
```
...
(time_period_classification = AQ_df %>%
  ...
  mutate(status = case_when(
    fire_period >= 85 & post_fire_period >= 85 & pre_fire_period >= 95 ~ "Complete data throughout fire period",
    fire_period < 75 & post_fire_period >= 75 & pre_fire_period >= 95 ~ "Sensor went offline during fire, (most data) returned online",
    fire_period < 75 & post_fire_period <= 75 & pre_fire_period >= 95 ~ "Sensor went offline during fire, (most data) did not return online",
    fire_period < 75 & post_fire_period <= 75 & is.na(pre_fire_period) ~ "Sensor added during fire without much data, (most data) did not return online",
...
# sensor 112606
pdf(file = "images/fire_map2.pdf")
(detailed_sensor_map = basemap + 
    geom_point(data=(plot_data %>% filter(ID != "112606")), aes(x=Lon,y=Lat, label = ID, shape = Location, color = status, label = rownames(ID)))) + scale_color_manual(values = c("light green","light blue","red","orange","purple"))
dev.off()
