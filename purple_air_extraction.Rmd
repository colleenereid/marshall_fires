...
for (sensor in 1:30) {
  ...
  if(length(sensor_xts$ID) > 0) {
    ...
    p <- ggplot(fortify(sensor_pm25a), aes(x=Index, y=pm25_a, color="black")) +
      ...
      labs(x="date",
           title = paste(sensor_info$Name, "- (", sensor_info$city, "-", sensor_info$Location,  ") : sensor", id, "pm2.5"))
    # print(p)
    ...
    p2 <- ggplot(fortify(sensor_temp), aes(x=Index, y=temp)) +
      geom_line() +
      labs(x="date",
           title = paste(sensor_info$Name, "- (", sensor_info$city, "-", sensor_info$Location,  ") : sensor", id, "temperature"))
    p3 <- ggplot(fortify(sensor_rh), aes(x=Index, y=rh)) +
    geom_line() +
    labs(x="date",
         title = paste(sensor_info$Name, "- (", sensor_info$city, "-", sensor_info$Location,  ") : sensor", id, "relative humidity"))
