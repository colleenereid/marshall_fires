...
(time_period_classification = AQ_df %>%
  ...
  mutate(Status = case_when(
    fire_period >= 75 & post_fire_period >= 87 & pre_fire_period >= 95 ~ "Complete data throughout fire period",
...
time_period_classification$Status = factor(time_period_classification$Status, levels = c("Complete data throughout fire period", "Sensor offline during fire, returned online", "Sensor offline during fire, did not return online", "Sensor added during fire, did not return online", "Sensor came online after fire"))
...
(detailed_sensor_map = basemap + 
    geom_point(data=(plot_data %>% filter(ID != "112606")), aes(x=Lon,y=Lat, label = ID, shape = Location, color = Status, size = 3, label = rownames(ID))) + 
    scale_color_manual(values = c("green","blue","red","orange","purple")) +
    theme_light() +
